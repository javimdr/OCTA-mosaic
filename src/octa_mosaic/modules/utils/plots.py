...
def impair(img_a, img_b, method="falsecolor", **kargs) -> np.ndarray:
    ...
    if method == "diff":
        difference = B.astype(np.int16) - A.astype(np.int16)  # type: ignore
        return (difference / 2 + 128).astype(np.uint8)
    ...
    raise ValueError("Unkown method. Options: falsecolor, diff, blend or chess")
...
def _fig_mult(
    ...
    for i in range(len(images)):
        ...
        if len(im.shape) == 2:
            ax.imshow(im, cmap="gray", aspect="equal", vmin=vmin, vmax=vmax)  # type: ignore
        else:
    ...
    #                             hspace=subplots_space, wspace=subplots_space)
    return fig
...
def save_mult(
    ...
    fig = _fig_mult(
        ...
        title,
        base_size=base_size,
        subplots_space=subplots_space,
    )
    fig.savefig(filename, facecolor="w", bbox_inches="tight")  # bbox_inches='tight',
...
def plot_mult_hist(
    images, labels, rows: int, cols: int, title: Optional[str] = None, base_size=6
):
    fig = plt.figure(figsize=(base_size * cols, base_size * rows))
    if title:
        fig.suptitle(title)
