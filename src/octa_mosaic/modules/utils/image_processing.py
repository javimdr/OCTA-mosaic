...
def imadjust(img: np.ndarray, contrast_limits: Tuple[int, int] = (1, 99)) -> np.ndarray:
    ...
    J = imadjust(I) maps the intensity values in grayscale image I to new values in J. By default, imadjust
    saturates the bottom 1% and the top 1% of all pixel values. This operation increases the contrast of the output
    image J.
    ...
    contrast_limits : Tuple[int, int], optional
        Contrast limits. Default is (1, 99).
...
def top_hat(img: np.ndarray, disk_radius: int) -> np.ndarray:
    ...
    im_bg = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel)
    im_bg = cv2.blur(im_bg, (disk_radius * 2, disk_radius * 2))
    out_img = cv2.subtract(img, im_bg)
    return out_img
...
def hessian_ridges(
    ...
    ridges_norm = cv2.normalize(
        ridges, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8UC1
    )
    ridges_norm = cv2.bitwise_not(ridges_norm)
    return ridges_norm
